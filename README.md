...
For now, content is generated by Claude and should be reviewed before publication.
...
Prints a structural overview and saves three files under `generated_content/<app>/`:
...
```text
egon/
├── cli.py               # CLI entry point (commands: generate, pack, generate-all, graph-report, list-packs)
...
├── graph.py             # Node graph analysis and metrics
